  ...
  return (
    <div className="min-h-[100vh] bg-[#F9F9F9] layout_container w-full flex flex-col justify-center items-center">
      {loading ? (
        ...
        <div className="w-full mt-14 md:w-[95%] lg:w-[80%] p-5 font-light bg-white sm:p-8 rounded-lg">
          <div className="w-full">
            <div className="flex justify-between items-start sm:mb-6">
              <div className="flex justify-start items-start">
                 <Button onClick={handleOpen} icon={arrowLeft} variant="white" className="w-10 h-10 !p-0" />
                <div className="mx-6 hidden sm:block">
                  <h1 className="text-[#1F1F1F] text-2xl font-bold">{data?.results[0].category}</h1>
                  <p className="text-[#667085] text-sm">Select the correct answer from the following</p>
                </div>
              </div>
              <div>
                <Button title="Restart" icon={reload} variant="white" onClick={handleRestart} />
              </div>
            </div>
            <div className="block sm:hidden w-full mb-6 mt-2">
                  <h1 className="text-[#1F1F1F] text-xl font-bold">{data?.results[0].category}</h1>
                  <p className="text-[#667085] text-sm">Select the correct answer from the following</p>
                </div>
            <Question
      ...
      <QuitModal title="Are you sure you want to go back?" type="back" open={openQuitModal} handleClose={handleClose} />
    </div>
  );
};
